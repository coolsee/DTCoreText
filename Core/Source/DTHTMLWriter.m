...
- (void)_buildOutputAsSimpleHTMLFragment:(BOOL)fragment
{
	// reusable styles
	_styleLookup = [[NSMutableDictionary alloc] init];
	...
	// ----- SPAN enumeration
	NSRange paragraphRange = NSMakeRange(0, [_attributedString length]);
...
- (NSString *)HTMLSimpleString
{
	if (!_HTMLString)
	{
		[self _buildOutputAsSimpleHTMLFragment:NO];
	}
...
- (NSString *)HTMLSimpleFragment
{
	if (!_HTMLString)
	{
		[self _buildOutputAsSimpleHTMLFragment:true];
	}
